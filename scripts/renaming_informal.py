...
Output naming:
  ...
  - <Event Name> is derived from the event folder name, stripping a
    leading numeric prefix ("01 "), a trailing "Dt-..." date stamp, and a
    trailing "Clips-NN" count.
...
-----
    ...
    python3 renaming_informal.py "/path/to/Tour Root"

    # Actually perform the renames:
    python3 renaming_informal.py "/path/to/Tour Root" --apply

    # Restrict to specific file extensions (default covers common
    # audio/video formats):
    python3 renaming_informal.py "/path/to/Tour Root" --ext mp4 mov wav

    # Write the full rename plan to a CSV for review/audit:
    python3 renaming_informal.py "/path/to/Tour Root" --csv plan.csv
"""
